...
// Shopify API versions (separadas por estabilidad)
const PRODUCT_API_VERSION = "2024-01";
const FULFILLMENT_API_VERSION = "2026-01";

// Rate limit control (2 rps aprox) => intervalo seguro 650ms
const SHOPIFY_MIN_INTERVAL_MS = 650;

// Retry control
const MAX_RETRIES = 6; // suficiente para picos
...
/* ==========================
   ...
   (misma lógica, pero con throttle+retry)
========================== */

async function transformProductById(shop, accessToken, productId) {
  // Espera corta para que Shopify "termine" de crear variantes
  await sleep(1200);
  ...
  // Update variantes: precio, sku, peso
  for (const variant of realVariants) {
    // Shopify espera precio con punto; acá siempre mandamos número final entero en MXN
    ...
    await shopifyRequest(shop, {
      ...
      data: {
        variant: {
          id: variant.id,
          price: String(mxnPrice), // entero en string (válido)
          sku: variant.sku,
  ...
  if (!locationId) {
    throw new Error("No locations found to set inventory");
  }

  // Inventory set por variante
  for (const variant of realVariants) {
    await shopifyRequest(shop, {
      method: "POST",
  ...
  log("Producto transformado correctamente con SEO leve", { shop, productId, variants: realVariants.length });
}
...
app.post("/webhook/products-create", async (req, res) => {
  // Respuesta inmediata (correcto para Shopify)
...
/* ==========================
   ...
   - Re-aplica transformaciones a product_ids
   - Útil para arreglar productos incompletos por 429 previos
========================== */
...
app.post("/reconcile", async (req, res) => {
  try {
    ...
    // Encolamos todos; la cola por shop garantiza orden y rate-limit safe
